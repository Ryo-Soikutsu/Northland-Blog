...
<summary>Source code for DrillBabyDrill.exe</summary>
...
Looking through the source code, we notice the following code
...
The function takes in a variable sum, performs a bitwise right shift by 8 bytes, and then XOR's each character in the key by the corresponding character in the encoded flag. The final output is than concatenated and returned. Looking through the rest of the code, it appears that the sum variable is calculated using the following code
...
The variable contains the product of all the x-coordinates of the player when they hit a bear. Since we already know part of the flag ("\*@flare-on.com"), we can do a direct brute-force on the key itself to recover the rest of the flag
...
def decode_with_key(key):
    ...
    for i in range(len(encoded)):
        out.append(chr(encoded[i] ^ (key + i)))
    return ''.join(out)

for k in range(256):
    s = decode_with_key(k)
    if all(32 <= ord(c) <= 126 for c in s):   # printable ASCII check
        if "@flare" in s or "drill" in s or s.startswith("drilling"):
            print(k, s)
...
Pretty easy challenge to start off the CTF.
